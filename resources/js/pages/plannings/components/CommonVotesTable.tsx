...
const CommonVotesTable: React.FC<CommonVotesTableProps> = ({ features, planningId }) => {
    ...
    const handleEditCommitmentModal = (feature: Feature, commitment: Commitment) => {
        ...
        if (statusValue === 'suggested') {
            ...
            transitions.push({ value: 'completed', label: 'Erledigt', color: 'bg-green-100 text-green-800' });
        }
    ...
    const recalcButton = (
        <button
            type="button"
            className={`rounded border border-blue-600 px-2 py-1 text-sm text-blue-600 hover:bg-blue-50 focus:outline-none ${loading ? 'cursor-not-allowed opacity-50' : ''}`}
    ...
    const successMsg = success && <span className="ml-2 text-xs text-green-600">Erfolgreich!</span>;
                                        ...
                                        {voteTypes.map((type) => (
                                            <TableHead
                                                ...
                                                onClick={() =>
                                                    setSort((s) =>
                                                        s && s.type === type
                                                            ? { type, direction: s.direction === 'asc' ? 'desc' : 'asc' }
                                                            : { type, direction: 'asc' },
                                                    )
                                                }
                                            >
